...
A few days ago I was enticed into thinking if there is an app for *generating* names (<strike>no, not a startup idea</strike>).
...
This post has 6 sections: The introduction just got over. We'll now take a quick look at the data, followed by a discussion on 
...
With 5-dimensional embeddings, "joe" will now be a matrix of 26 rows and five columns.

The following three lines of code is all that's needed add embeddings. ```embeddings``` is a matrix, which has one row for 
