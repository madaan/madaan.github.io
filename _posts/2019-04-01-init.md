...
The subsequent discussion outlines a solution to the problem of exploding and vanishing magnitude we saw above. The solution is the well known [Xavier Initialization](http://proceedings.mlr.press/v9/glorot10a.html).  As we'll see, Xavier Initialization is sufficient to solve the problem at hand. However, that's because we did not use any activation function. If we had used, say a ReLu, the more recent [Kaiming Initialization](https://arxiv.org/abs/1502.01852) would have been more effective. 
 

The Xavier initialization tells us that we should use a scaling factor of `math.sqrt(n_in)` where `n_in` is the number of inputs of our matrix (512 in our case). Recall that in the attempted solution above, we had used a `scaling_factor` of 100 (and it had reduced the product to 0, indicating it was a bit too much). Let's see if Xavier init helps:
