...
The solution to the problem of exploding and vanishing magnitude we saw above is the well known [Xavier Initialization](http://proceedings.mlr.press/v9/glorot10a.html).  
 

The Xavier initialization suggests using a scaling factor of $$\sqrt(n\_in)$$, where `n_in` is the number of inputs to the matrix (or the dimension that's common with the vector the matrix is being multiplied with). In our case, the number of inputs to the matrix $$a$$ is 512. Thus, the scaling factor should be $$\sqrt(512)$$.

Let's see if the Xavier init helps:
