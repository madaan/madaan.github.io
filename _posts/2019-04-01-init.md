...
The problem of weight initialization is motivated by a simulation. The real cause of ill-behaved multiplication output is identified, and scaling the weight matrix using the Xavier initialization is presented as a solution. The rest of the writeup then provides experiments and proofs to explain why such an initialization worked.  The blog was written as part of a discussion for part 2 of the [Fast AI course](https://course.fast.ai/index.html) (Spring 2019 session). 
  
# Motivation

Training a neural network involves a bunch of operations, and one of the most common of these operations is multiplication. Often, the multiplication happens between a bunch of numbers, represented as matrices. In the case of _deep_ neural networks, we end up with a long sequence of multiplications.
...
As seen above, a neural network is just a series of multiplications.  To begin our investigation, let's take a random input vector, ``x`` and a random matrix ``a``.  Note that the numbers are sampled from a [normal distribution](https://pytorch.org/docs/stable/torch.html#torch.randn) with a mean 0 and variance 1 or as it is popularly known, $\mathcal{N}(0, 1)$. 

We'll multiply ``x`` and ``a``  100 times (as if the network had 100 layers), and see what gets out on the other side. **Note that we don't use any activation function for the sake of simplicity**. 
...
The subsequent discussion outlines a solution to the problem listed above. The solution is the well known [Xavier Initialization](http://proceedings.mlr.press/v9/glorot10a.html).  As we'll see, it suffices to solve the case at hand. However, that's because we did not use any activation function. If we had used, say a ReLu, the more recent [Kaiming Initialization](https://arxiv.org/abs/1502.01852) would have come in handy. 
 

The Xavier initialization tells us that we should use a scaling factor that equals `math.sqrt(n_in)` where `n_in` is the number of inputs of our matrix. Recall that in the attempted solution above, we had used a `scaling_factor` of 100 (and it had reduced the product to 0). Let's see if Xavier init helps:
...
x = a @ x
```
We are not changing ``a`` at all. Thus, the only element that can give trouble is ``x``, since it's being updated. Seems like at some point in the multiplication, ``x`` starts getting high values, and thus the subsequent multiplications keep making things worse. To closely examine this phenomenon, let us denote the product of ``a`` and ``x`` by ``y``.  $$y = a * x$$ As in our running examples, if ``a`` is a matrix of size  512 x 512, and ``x`` is a vector of size 512, the output ``y`` is a vector of the size 512. 
...
To compute one element of ``y``, we sum 512 products of one element of ``a`` by one element of ``x``.  What's the mean and the variance of such a product? As we show later,  as long as the elements in ``a`` and the elements in ``x`` are [independent](https://en.wikipedia.org/wiki/Independence_(probability_theory)) (which they are in this case; one doesn't affect the other), the mean is ``0`` and the variance is ``512``. That is, each element of ``y`` is now taking the values _as if_ it was picked $\mathcal{N}(0, 512)$. This can also be seen experimentally as in the following code snippet. To avoid one-off errors, we repeat the experiment for `10000` iterations.
...
In other words, each element of y is now picked from an erratic distribution, and that's happening because we are adding a product of 512 elements, each picked from $\mathcal{N}(0, 1)$.  We keep feeding these ``y`` elements again in the loop as the input, and thus things go south soon.

  

Now, if we scale the weights of the matrix $a$ and divide them by this $math.sqrt(512)$, we will be picking elements of a from a normal distribution with $0$ mean and variance = $512$ or $\mathcal{N}(0, 512)$ (proof later). 


This scaling will, in turn, give us a distribution of $y$ in which each element has 0 mean and std = 1, thus allowing us to repeat the product has many times as we want. This is **NOT** different from the intuitive solution we had discussed earlier. We were right in sensing that scaling one of the participants in the product may help, and it did. Xavier init helped us in finding out the *exact magnitude* of the scaling factor: $\sqrt(512)$, instead of what we had initially used: 100. 
...
([Reference](https://stats.stackexchange.com/questions/52646/variance-of-product-of-multiple-random-variables))
...
## Summary
Chained multiplication of matrices lies at the core of neural networks. We see that without proper initialization, inputs sampled from well-behaved distribution $$( \mathcal{N}(0, 1))$$  will vanish (over-scaling) or explode (under-scaling). Dividing weight matrix by $$\sqrt(num\_inputs)$$ (512 in the running example), known as  [Xavier Initialization](http://proceedings.mlr.press/v9/glorot10a.html), helps in ensuring that the output of each of the multiplications is well-behaved, thus ensuring that the sequence of multiplications yields a reasonable output at each step. Thus allowing the training of deep neural networks possible. 
